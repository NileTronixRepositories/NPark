...
namespace CRM.Infrastructure.Token
{
    public class JwtProvider : IJwtProvider
    {
        ...
        public JwtProvider(IGenericRepository<SuperAdmin> repository, IOptions<JwtOption> options, CRMDBContext context, IGenericRepository<Account> accountrepository)
        {
    ...
    SecurityAlgorithms.HmacSha256);
        ...
        public async Task<UserTokenDto> GenerateAccount(Account user, CancellationToken cancellationToken = default)
        {
            var userEntity = await _context.Set<Account>()
                .Where(x => x.Id == user.Id)
                .Include(x => x.Role)
                .ThenInclude(r => r.GetPermissions)
                .ThenInclude(p => p.Permission)
